...
def sorted_named_shape_tuples(named_shape_tuples):
  return sorted(set(named_shape_tuples),
                key=lambda x: x if isinstance(x, str) else hash(x))

def canonicalize_named_shape(named_shape):
  return dict(sorted_named_shape_tuples(named_shape.items()))

def join_named_shapes(*named_shapes):
  named_shape_tuples = sorted_named_shape_tuples(
...
def typecompat(aval_ref: AbstractValue, aval: AbstractValue) -> bool:
  ...
  Ignores weak_type and named_shape.
  """
  try:
...
def typematch(aval1: AbstractValue, aval2: AbstractValue) -> bool:
  ...
  Ignores weak_type and named_shape.
  """
  aval1 = raise_to_shaped(aval1, weak_type=False).strip_named_shape()
  aval2 = raise_to_shaped(aval2, weak_type=False).strip_named_shape()
  return aval1 == aval2
